...
export default function GapNoIntegrationsWithPublicDataApisBls() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-integrations-with-public-data-apis-bls-un/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-integrations-with-public-data-apis-bls-un', project: 'AISupplyChainReshoringAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-integrations-with-public-data-apis-bls-un/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No integrations with public data APIs (BLS, UN Comtrade, trade databases)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AISupplyChainReshoringAdvisor</p>
      </div>
