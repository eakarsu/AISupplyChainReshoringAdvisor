...
export default function CfMonteCarloScenarioPlanningForTariffLabor() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-monte-carlo-scenario-planning-for-tariff-labor-currency/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-monte-carlo-scenario-planning-for-tariff-labor-currency', project: 'AISupplyChainReshoringAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-monte-carlo-scenario-planning-for-tariff-labor-currency/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Monte Carlo scenario planning for tariff/labor/currency sensitivity</h1>
