...
export default function CfLaborArbitrageModelingComparingWageProductivityTraining() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-labor-arbitrage-modeling-comparing-wage-productivity-training-costs/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-labor-arbitrage-modeling-comparing-wage-productivity-training-costs', project: 'AISupplyChainReshoringAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-labor-arbitrage-modeling-comparing-wage-productivity-training-costs/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Labor arbitrage modeling comparing wage/productivity/training costs across geographies</h1>
