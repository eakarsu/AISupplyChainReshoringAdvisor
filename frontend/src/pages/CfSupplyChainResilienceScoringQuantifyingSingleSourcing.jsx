...
export default function CfSupplyChainResilienceScoringQuantifyingSingleSourcing() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-supply-chain-resilience-scoring-quantifying-single-sourcing-and-geopolitical/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-supply-chain-resilience-scoring-quantifying-single-sourcing-and-geopolitical', project: 'AISupplyChainReshoringAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-supply-chain-resilience-scoring-quantifying-single-sourcing-and-geopolitical/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Supply chain resilience scoring quantifying single-sourcing and geopolitical exposure</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AISupplyChainReshoringAdvisor</p>
      </div>
