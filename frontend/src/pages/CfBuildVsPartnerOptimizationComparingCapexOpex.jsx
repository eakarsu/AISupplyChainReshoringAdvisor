...
export default function CfBuildVsPartnerOptimizationComparingCapexOpex() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-build-vs-partner-optimization-comparing-capex-opex-vs-continued-outsourcing/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-build-vs-partner-optimization-comparing-capex-opex-vs-continued-outsourcing', project: 'AISupplyChainReshoringAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-build-vs-partner-optimization-comparing-capex-opex-vs-continued-outsourcing/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Build-vs-partner optimization comparing capex+opex vs continued outsourcing</h1>
