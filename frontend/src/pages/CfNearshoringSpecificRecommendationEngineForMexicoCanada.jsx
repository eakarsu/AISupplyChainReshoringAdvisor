...
export default function CfNearshoringSpecificRecommendationEngineForMexicoCanada() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-nearshoring-specific-recommendation-engine-for-mexico-canada/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-nearshoring-specific-recommendation-engine-for-mexico-canada', project: 'AISupplyChainReshoringAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-nearshoring-specific-recommendation-engine-for-mexico-canada/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Nearshoring-specific recommendation engine for Mexico/Canada</h1>
