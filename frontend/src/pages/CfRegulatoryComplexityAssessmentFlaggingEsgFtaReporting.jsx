...
export default function CfRegulatoryComplexityAssessmentFlaggingEsgFtaReporting() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-regulatory-complexity-assessment-flagging-esg-fta-reporting-burden/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-regulatory-complexity-assessment-flagging-esg-fta-reporting-burden', project: 'AISupplyChainReshoringAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-regulatory-complexity-assessment-flagging-esg-fta-reporting-burden/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Regulatory complexity assessment flagging ESG/FTA reporting burden by geography</h1>
