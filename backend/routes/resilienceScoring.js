...
// Supply chain resilience scoring quantifying single-sourcing and geopolitical exposure
// Feature: resilience-scoring
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AISupplyChainReshoringAdvisor" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Supply chain resilience scoring quantifying single-sourcing and geopolitical exposure.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'resilience-scoring', payload, result);
    res.json({ success: true, feature: 'resilience-scoring', ...result });
  } catch (err) {
    console.error('resilience-scoring analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AISupplyChainReshoringAdvisor". Return strict JSON only.',
      `Feature: Supply chain resilience scoring quantifying single-sourcing and geopolitical exposure. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'resilience-scoring', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'resilience-scoring', title: 'Supply chain resilience scoring quantifying single-sourcing and geopolitical exposure', project: 'AISupplyChainReshoringAdvisor' });
});
