...
// Monte Carlo scenario planning for tariff/labor/currency sensitivity
// Feature: monte-carlo-scenarios
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Monte Carlo scenario planning for tariff/labor/currency sensitivity.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'monte-carlo-scenarios', payload, result);
    res.json({ success: true, feature: 'monte-carlo-scenarios', ...result });
  } catch (err) {
    console.error('monte-carlo-scenarios analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Monte Carlo scenario planning for tariff/labor/currency sensitivity. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'monte-carlo-scenarios', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'monte-carlo-scenarios', title: 'Monte Carlo scenario planning for tariff/labor/currency sensitivity', project: 'AISupplyChainReshoringAdvisor' });
});
