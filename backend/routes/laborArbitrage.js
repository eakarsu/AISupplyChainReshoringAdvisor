...
// Labor arbitrage modeling comparing wage/productivity/training costs across geographies
// Feature: labor-arbitrage
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Labor arbitrage modeling comparing wage/productivity/training costs across geographies.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'labor-arbitrage', payload, result);
    res.json({ success: true, feature: 'labor-arbitrage', ...result });
  } catch (err) {
    console.error('labor-arbitrage analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Labor arbitrage modeling comparing wage/productivity/training costs across geographies. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'labor-arbitrage', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'labor-arbitrage', title: 'Labor arbitrage modeling comparing wage/productivity/training costs across geographies', project: 'AISupplyChainReshoringAdvisor' });
});
